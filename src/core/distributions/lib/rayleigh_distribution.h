#ifndef RAYLEIGH_DISTRIBUTION_H
#define RAYLEIGH_DISTRIBUTION_H

#include "distribution_interface.h"

#ifdef __cplusplus
extern "C" {
#endif

/**
 * @brief Rayleigh distribution PDF calculation
 * @param x The value at which to evaluate the PDF
 * @param params Array containing [scale]
 * @param param_count Number of parameters (should be 1)
 * @return PDF value at x
 */
double rayleigh_pdf(double x, double* params, int param_count);

/**
 * @brief Rayleigh distribution CDF calculation
 * @param x The value at which to evaluate the CDF
 * @param params Array containing [scale]
 * @param param_count Number of parameters (should be 1)
 * @return CDF value at x
 */
double rayleigh_cdf(double x, double* params, int param_count);

/**
 * @brief Validate Rayleigh distribution parameters
 * @param params Array containing [scale]
 * @param param_count Number of parameters (should be 1)
 * @return 1 if parameters are valid, 0 otherwise
 */
int rayleigh_validate_params(double* params, int param_count);

/**
 * @brief Get the Rayleigh distribution interface
 * @return Pointer to the distribution_t structure for Rayleigh distribution
 */
const distribution_t* get_rayleigh_distribution(void);

#ifdef __cplusplus
}
#endif

#endif // RAYLEIGH_DISTRIBUTION_H