#ifndef PARETO_DISTRIBUTION_H
#define PARETO_DISTRIBUTION_H

#include "distribution_interface.h"

#ifdef __cplusplus
extern "C" {
#endif

/**
 * @brief Pareto distribution PDF calculation
 * @param x The value at which to evaluate the PDF
 * @param params Array containing [scale, shape]
 * @param param_count Number of parameters (should be 2)
 * @return PDF value at x
 */
double pareto_pdf(double x, double* params, int param_count);

/**
 * @brief Pareto distribution CDF calculation
 * @param x The value at which to evaluate the CDF
 * @param params Array containing [scale, shape]
 * @param param_count Number of parameters (should be 2)
 * @return CDF value at x
 */
double pareto_cdf(double x, double* params, int param_count);

/**
 * @brief Validate Pareto distribution parameters
 * @param params Array containing [scale, shape]
 * @param param_count Number of parameters (should be 2)
 * @return 1 if parameters are valid, 0 otherwise
 */
int pareto_validate_params(double* params, int param_count);

/**
 * @brief Get the Pareto distribution interface
 * @return Pointer to the distribution_t structure for Pareto distribution
 */
const distribution_t* get_pareto_distribution(void);

#ifdef __cplusplus
}
#endif

#endif // PARETO_DISTRIBUTION_H