...
| Beta分布 | $\alpha$, $\beta$ | 比例参数估计      | $f(x) = \frac{x^{\alpha-1}(1-x)^{\beta-1}}{B(\alpha,\beta)},\ 0 \leq x \leq 1$ |
| Chi-Square分布 | $k$ | 拟合优度检验 | $f(x;k) = \frac{x^{(k/2)-1}e^{-x/2}}{2^{k/2}\Gamma(k/2)}$ |
| F分布 | $d_1$, $d_2$ | 方差分析 | $f(x;d_1,d_2) = \sqrt{\frac{(d_1x)^{d_1}d_2^{d_2}}{(d_1x+d_2)^{d_1+d_2}}} \Bigg/ x B(\frac{d_1}{2},\frac{d_2}{2})$ |
| Student-t分布 | $\nu$ | 假设检验 | $f(t) = \frac{\Gamma((\nu+1)/2)}{\sqrt{\nu\pi}\Gamma(\nu/2)}(1+t^2/\nu)^{-(\nu+1)/2}$ |
| Pareto分布 | $x_m$, $\alpha$ | 社会经济建模 | $f(x) = \frac{\alpha x_m^\alpha}{x^{\alpha+1}}$ |
| Rayleigh分布 | $\sigma$ | 信号处理 | $f(x) = \frac{x}{\sigma^2}e^{-x^2/(2\sigma^2)}$ |
| Weibull分布 | $k$, $\lambda$ | 可靠性分析 | $f(x) = \frac{k}{\lambda}(\frac{x}{\lambda})^{k-1}e^{-(x/\lambda)^k}$ |
